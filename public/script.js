...
function buildFilters() {
  ...
  HEADERS.filter(c => FILTER_COLUMNS.has(c)).forEach(col => {
    ...
    if (col === 'Type') {
      ...
      const ph = col === 'Called Time' ? 'dd/mm/yyyy, hh:mm:ss' : `Search ${col}`;
      wrapper.innerHTML = `<div class="field"><label class="label is-small">${col}</label><input data-col="${col}" id="filter_${colId}" class="input is-small" type="text" placeholder="${ph}"></div>`;
    }
